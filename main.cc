...
int main(){
	...
	bool shutdown = false;
	const int b_length = main_sound_stream->get_pcm_buffer_length();
	...
	while (!shutdown){
		...
		main_sound_stream->get_pcm_buffer();
		for(int i=0; i<b_length; i++){
			std::cout << main_sound_stream->get_pcm_buffer()[i] << " ";
		}
